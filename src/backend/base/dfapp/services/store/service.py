...
from dfapp.services.store.exceptions import APIKeyError, FilterError, ForbiddenError
from dfapp.services.store.schema import (
...
from dfapp.services.store.utils import (
...
def get_id_from_search_string(search_string: str) -> Optional[str]:
    ...
    if "www.dfapp.store/store/" in search_string:
...
class StoreService(Service):
    """This is a service that integrates dfapp with the store which
