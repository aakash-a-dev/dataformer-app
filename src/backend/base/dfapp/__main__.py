...
def print_banner(host: str, port: int):
    ...
    # Attempt to handle dfappbase similarly
    if dfapp_version is None:  # This means dfapp.version was not imported
        try:
            from importlib import metadata

            dfapp_base_version = metadata.version("dfappbase")
            is_pre_release |= is_prerelease(dfapp_base_version)  # Update pre-release status
            notice = build_version_notice(dfapp_base_version, "dfappbase")
            notice = stylize_text(notice, "dfappbase", is_pre_release)
            if notice:
                notices.append(notice)
            package_names.append("dfappbase")
            package_name = "DFAPP Base"
...
def run_dfapp(host, port, log_level, options, app):
    ...
    try:
        if platform.system() in ["Windows"]:
            ...
            from dfapp.server import DfappApplication

            DfappApplication(app, options).run()
    except KeyboardInterrupt:
        pass
